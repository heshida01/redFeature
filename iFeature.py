...
if __name__ == '__main__':
	...
	pd_data = pd.read_table(outFile,sep='\t',)
	col = pd_data.columns
	...
	i = 0
	for x in np_data[:,0]:
		np_data[:,0][i] = 1 if 'The' in x else 0
		i += 1
		#datas.to_csv(outFile+'.csv',index=None)
		#print(datas)
	#pd.to_csv(np_data,columns = col)
	df=pd.DataFrame(np_data,columns=col)
	df.to_csv('AAC.csv',index=None)